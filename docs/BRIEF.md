...
**Деплой:** поддомен на avpdev.com (например vibecoding.avpdev.com), аналогично прецеденту business-up.avpdev.com.
...
9. **AdGenerationService** — генерация headlines/descriptions через `AdGeneratorInterface`:
   ...
   - `LlmAdGenerator` — реализуется через DeepSeek API (демонстрация гибридного подхода: LLM-генерация с автоматическим fallback на `TemplateAdGenerator` при недоступности API/timeout), переключение через DI-конфиг. Дальнейшее развитие (интеграция с локальными self-hosted моделями, мультивариантная генерация под A/B) — в Roadmap.

   Это архитектурное решение — явный сигнал масштабируемости: смена генератора = смена одной DI-биндинги в конфиге, ноль изменений в остальном пайплайне.
...
## 6. Что явно показать в сопроводительном письме/README к решению

**Документация билингвальная (EN + RU)** — команда интернациональная, README должен быть понятен без перевода. Формат: либо `README.md` (en) + `README.ru.md`, либо один файл с чёткими секциями `## English` / `## Русский` — на усмотрение агента, но обе версии обязательны и синхронны по содержанию (не "en — подробно, ru — тезисно").

- Короткое объяснение логики (2-3 абзаца): почему intent/audience_segment важны, зачем gap analysis, почему pipeline, а не монолит
- Ссылка на живой URL + тестовые данные внутри (не просить их присылать свои)
- **Vibecoding Log** (отдельный раздел README, это то, что они прямо просили оценить):
  - "Архитектура и схема данных спроектированы человеком, реализация и тесты — AI-агентами (Claude Code)"
  - Краткая хронология: бриф → фазы → ревью на каждой фазе → деплой, с примерной затраченной длительностью (от брифа до деплоя: X часов)
  - Опционально: приложить ключевые промпты/цепочку задач по фазам — показывает не "я нажал génерировать", а управляемый инженерный процесс
