...
- **Интеграция с локальными self-hosted моделями** — как альтернатива/дополнение к уже реализованному DeepSeek API, для генерации объявлений без зависимости от внешнего облачного провайдера.
...
- **Внутренний парсер сайта site.pro для автообнаружения направлений.** Сейчас маппинг категория→target URL захардкожен (6 продуктовых направлений вручную в коде) — при росте продукта список придётся поддерживать руками. Идея: парсер проходит по `sitemap.xml` сайта, извлекает осмысленный контент каждой страницы (title, meta description, заголовки), прогоняет через AI (переиспользуя уже подключённый DeepSeek) для классификации — к какому направлению относится страница, есть ли явное коммерческое назначение. Результат — предложение новых category/target_url пар, которые ещё не покрыты текущей таксономией, плюс рекомендации по направлениям для будущих кампаний.

  Важные технические детали на будущее: периодический запуск (не на каждый импорт, а по крону — сайт не меняется каждый час), вежливый краулинг с уважением к `robots.txt` и задержкой между запросами, кэширование по хэшу контента страницы (не гонять AI повторно на неизменившиеся страницы), и главное — результат должен быть предложением на ревью человеку (по аналогии с текущим Gap Analysis), а не автоматической подменой таксономии без контроля.
